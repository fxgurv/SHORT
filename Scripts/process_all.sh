...
# Script to generate & Upload videos to YT Shorts automatically
...
while true; do
    ...
    echo "All accounts processed. Starting 10-minute countdown..."
    
    # 10-minute countdown (600 seconds)
    for ((i=600; i>0; i--)); do
        minutes=$((i/60))
        seconds=$((i%60))
        printf "\rNext round in: %02d:%02d" $minutes $seconds
        sleep 1
    done
    echo -e "\nRestarting process..."
done
